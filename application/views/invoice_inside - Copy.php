					...
					<div class="challan-div">			     
						<form id="insider_invoice_form" name="insider_invoice_form" class="form-horizontal" action="<?php echo site_url('Invoice/add_insider_invoice');?>" method="post">
							...
							<div class="form-group" id="table_with_tax">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <tr class="row_one">
                                            <td>
                                                <select name="items[]" id="items" class="items form-control">
                                            ...
                                            <td><input type="text" name="stk[]" class="stk form-control" size="3" maxlength="7" value=""></td>
                                            <td><input type="text" name="prod_ID" class="prod form-control" size="3" maxlength="7" value=""></td>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-3">
                                    <input type="text" name="discount" class="form-control only_num" id="discount" style="display: inline; width: 50%" value="" size="3">
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-sm-2 col-sm-offset-6">
...
$(document).ready(function(){
    ...
    $('#table_with_tax, #table_without_tax,.in_state').hide();
    setTimeout(function(){
        ...
        if(a_w_t != '')
        {
            ...
            { 
               // $('#insider_invoice_form')[0].reset(); 
                $('#items, .qnty, .rate, .amount, #discount, #other_charge, #total_tax_value, #cgst_charge, #sgst_charge, #igst_charge, #total_amount, #total_round, #total_word').val('');              
                $('#table_with_tax').show();  
                $('#table_without_tax').hide();
                //show AMOUNT by qnty*rate
                $('.amount').on('focus', function(){                    
                    var ro  = $(this).parents('tr');
                    var amount_with_tax = ro.find("input[name*= 'amount_with_tax']").val();
                    ...
                    var tot_amount = the_amount + amount_with_tax;
                    $(this).val(tot_amount);        
            ...
            {
               // $('#insider_invoice_form')[0].reset();    
               $('#items, .qnty, .amount_with_tax, .rate, .amount, #discount, #other_charge, #total_tax_value, #cgst_charge, #sgst_charge, #igst_charge, #total_amount, #total_round, #total_word').val('');                         
                $('#table_with_tax').hide();  
            ...
            $('#total_tax_value').on('focus', function(){ 
                ...
                total +=  parseFloat(other_charge) - parseFloat(discount);  
                if(total != crnt_val)
                {
    ...
    $('.items').change(function(){ 
                ...
                console.log(p_id);
                $.ajax({
                    url : "<?php echo base_url('/index.php/Product/getProductDetail');?>",
                    method : "POST",
                    data : {p_id: p_id},
                    async : true,
                    dataType : 'json',
                    success: function(data){
                        console.log(data);
                        $('.stk').val(data[0]['stock']);
                        $('.rate').val(data[0]['price']);
                        $('.prod').val(data[0]['price']);
                    }
                });
                return true;
            }); 
            $('.stk').attr("disabled", true) 
